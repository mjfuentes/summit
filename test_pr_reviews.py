...
"""
    ...
    engineer_review = await pr_review_system.generate_review("engineer", mock_pr_info, mock_file_changes)
    print("\n" + "="*60)
...
async def test_multi_role_review_on_current_pr():
    ...
    try:
        ...
        if result.get("success"):
            ...
            print("="*60)
            print(f"Check the PR at: https://github.com/{owner}/{repo}/pull/{pr_number}")
...
"""
    ...
    for persona in personas:
        ...
        review = await pr_review_system.generate_review(persona, pr_info, file_changes)
        if review:
...
async def main():
    ...
    if not os.getenv("ANTHROPIC_API_KEY"):
        print("WARNING: ANTHROPIC_API_KEY not found. AI reviews will not work.")
    
    if not os.getenv("GITHUB_TOKEN"):
        print("WARNING: GITHUB_TOKEN not found. Cannot post reviews to GitHub.")
    ...
    try:
        ...
        if choice == "4":
            confirm = input("\nThis will post reviews to PR #1. Continue? (y/N): ").strip().lower()
            if confirm == 'y':
        ...
        print("="*60)
        print("\nNext steps:")
        print("1. The multi-role review system is integrated into autonomous_server.py")
        print("2. When agents create PRs, they automatically get multi-perspective reviews")
        print("3. Each reviewer provides expertise-focused feedback")
        print("4. Reviews help ensure code quality, security, and business alignment")
