...
class TestSummit:
    ...
    def test_get_github_repo_info_from_env(self):
        ...
        with patch.dict(os.environ, {'GITHUB_OWNER': 'test-owner', 'GITHUB_REPO': 'test-repo'}):
    ...
    @pytest.mark.skip(reason="Temporarily disabled - needs cost_tracker API fix")
    @pytest.mark.asyncio
    async def test_get_advice_from_claude_success(self):
        """Test getting advice from Claude successfully"""
        mock_client = Mock()
        mock_message = Mock()
        mock_message.content = [Mock(text="Test advice response")]
        ...
        with patch('summit.get_anthropic_client', return_value=mock_client):
            with patch('summit.cost_tracker') as mock_tracker:
                ...
                mock_tracker.track_request.return_value = None
    ...
    @pytest.mark.skip(reason="Temporarily disabled - needs codespace data format fix")
    @pytest.mark.asyncio
    async def test_create_codespace_success(self):
        ...
        with patch('summit.get_github_headers', return_value={'Authorization': 'Bearer test'}):
    ...
    async def test_create_pull_request_success(self):
        ...
        with patch('summit.get_github_headers', return_value={'Authorization': 'Bearer test'}):
            with patch('requests.post', return_value=mock_response):
                result = await create_pull_request('owner', 'repo', 'Test PR', 'feature-branch')
    ...
    async def test_plan_capability_implementation_success(self):
        ...
        with patch('summit.get_anthropic_client', return_value=mock_client):
            with patch('summit.get_github_repo_info', return_value=('owner', 'repo')):
