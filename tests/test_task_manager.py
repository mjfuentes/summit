...
class TestTaskManager:
    ...
    async def test_update_task_status_basic(self):
        ...
        with patch('task_manager.get_database', return_value=mock_db):
            await update_task_status('test-123', 'running')
            
            mock_db.update_task.assert_called_once_with('test-123', {'status': 'running'})
    ...
    async def test_update_task_status_with_progress(self):
        ...
        with patch('task_manager.get_database', return_value=mock_db):
            ...
            mock_db.update_task.assert_called_once_with('test-123', {
                'status': 'running',
                'progress': '50%'
            })
    ...
    async def test_update_task_status_with_all_params(self):
        ...
        with patch('task_manager.get_database', return_value=mock_db):
            await update_task_status('test-123', 'failed', progress='75%', error='Test error')
    ...
    async def test_add_task_log_existing_logs(self):
        ...
        with patch('task_manager.get_database', return_value=mock_db):
            ...
            mock_db.get_task.assert_called_once_with('test-123')
            mock_db.update_task.assert_called_once_with('test-123', {
                'logs': ['Previous log', 'New log message']
            })
    ...
    async def test_update_task_container_info(self):
        ...
        with patch('task_manager.get_database', return_value=mock_db):
            await update_task_container_info('test-123', 'container-456', 'https://test.url')
