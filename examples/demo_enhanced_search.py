...
async def demo_enhanced_search():
    ...
    for query_info in search_queries:
        ...
        result = await handle_call_tool(
            "summit_learn", {"query": query_info["query"], "max_results": 3}
        )
    ...
    analytics_result = await handle_call_tool(
        "summit_analytics", {"include_suggestions": True}
    )
    print(" Search Analytics:")
    print(
        analytics_result[0].text[:500] + "..."
        if len(analytics_result[0].text) > 500
        else analytics_result[0].text
    )
    ...
    print(
        insights_result[0].text[:500] + "..."
        if len(insights_result[0].text) > 500
        else insights_result[0].text
    )
    ...
    no_match_result = await handle_call_tool(
        "summit_learn",
        {"query": "blockchain cryptocurrency trading", "max_results": 2},
    )
    print("   " + no_match_result[0].text[:300] + "...")

    # Try focused search
    print(f"\n Testing focused search with patterns analysis:")
    focused_result = await handle_call_tool(
        "summit_learn",
        {
    ...
    print(" Summit Status:")
    print(
        "   " + status_result[0].text[:400] + "..."
        if len(status_result[0].text) > 400
        else "   " + status_result[0].text
    )
    ...
    print(
        "   Set OPENAI_API_KEY for full semantic vector search capabilities."
    )
