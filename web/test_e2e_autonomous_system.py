...
class AutonomousSystemTest:
    ...
    def get_container_status(self, container_name):
        ...
        try:
            ...
            result = subprocess.run(['docker', 'ps', '-f', f'name={container_name}', '--format', 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'], 
    ...
    def stream_container_logs(self, container_name, duration_seconds=30):
        ...
        try:
            ...
            result = subprocess.run(['docker', 'logs', '--tail', '10', container_name], 
            ...
            # Stream new logs
            process = subprocess.Popen(['docker', 'logs', '-f', '--since', '30s', container_name], 
    ...
    def list_claude_containers(self):
        ...
        try:
            result = subprocess.run(['docker', 'ps', '-a', '--filter', 'name=claude'], 
                                  capture_output=True, text=True, timeout=5)
            if result.returncode == 0 and result.stdout.strip():
                lines = result.stdout.strip().split('\n')
            ...
            result = subprocess.run(['docker', 'ps', '-a', '--filter', 'since=2m'], 
                                  capture_output=True, text=True, timeout=5)
            if result.returncode == 0 and result.stdout.strip():
                lines = result.stdout.strip().split('\n')
                if len(lines) > 1:
    ...
    def check_docker_images(self):
        ...
        try:
            result = subprocess.run(['docker', 'images', '--filter', 'reference=claude-code-task'], 
                                  capture_output=True, text=True, timeout=5)
            if result.returncode == 0:
                ...
                if len(lines) > 1:  # Header + at least one image
                    ...
                    self.log("Claude Code Docker image NOT found - container build may be in progress")
    ...
    def check_claude_in_container(self, container_name):
        ...
        self.log(f"Checking Claude Code installation in container: {container_name}")
        ...
        try:
            ...
            result = subprocess.run(['docker', 'exec', container_name, 'which', 'claude'], 
                                  capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                self.log("  Claude Code CLI found in container")
                
                # Check version
                result = subprocess.run(['docker', 'exec', container_name, 'claude', '--version'], 
                                      capture_output=True, text=True, timeout=10)
                if result.returncode == 0:
                    self.log(f"  Claude version: {result.stdout.strip()}")
                else:
                    self.log("  Warning: Claude command found but version check failed")
                    
                # Check if API key is available
                result = subprocess.run(['docker', 'exec', container_name, 'env'], 
                                      capture_output=True, text=True, timeout=5)
                if result.returncode == 0 and 'ANTHROPIC_API_KEY' in result.stdout:
    ...
    def create_task(self):
        ...
        task_data = {
            ...
            "repository_url": repo_url if not self.debug_mode else None,  # Use repo only for git testing
            "github_token": None,
    ...
    def monitor_task(self, max_wait_minutes=15):
        ...
        self.log(f"Monitoring task {self.task_id} (timeout: {max_wait_minutes} minutes)...")
        ...
        while time.time() - start_time < max_wait:
            ...
            try:
                ...
                response = requests.get(f"{self.server_url}/api/tasks/{self.task_id}", timeout=15)
                ...
                if response.status_code == 200:
                    ...
                    if result.get('success'):
                        ...
                        if 'claude_code_url' in task and task['claude_code_url']:
                            web_url = task['claude_code_url']
                            if status == "running" and elapsed_time > 30:
                                self.log(f"Web terminal available: {web_url}")
                                self.log("  You can open this URL to see Claude Code in action!")
                        
                        # Log status changes
                        if status != last_status:
                            self.log(f"Task status: {status} - {progress} ({elapsed_time:.1f}s elapsed)")
                            if container_name and status == "running":
                                container_status = self.get_container_status(container_name)
                                self.log(f"Container status: {container_status}")
                            last_status = status
                        ...
                        if len(logs) > last_log_count:
                            ...
                            for log_entry in new_logs:
                                ...
                                elif any(keyword in log_entry.lower() for keyword in ['claude', 'error', 'failed', 'completed', 'terminal']):
                        ...
                        if int(elapsed_time) % 60 == 0 and elapsed_time > 60:
                            self.log(f"Still monitoring... {elapsed_time:.0f}s elapsed, status: {status}")
                            
                            # Check container health if we haven't done it yet and container is running
                            if container_name and status == "running" and not logs_streamed and elapsed_time > 90:
                                self.log("Task taking longer than expected, checking container health...")
                                self.check_claude_in_container(container_name)
                                self.stream_container_logs(container_name, duration_seconds=20)
                                logs_streamed = True
                            
                        # For debug mode, show more frequent updates
                        elif self.debug_mode and int(elapsed_time) % 30 == 0 and elapsed_time > 30:
                            self.log(f"Still monitoring... {elapsed_time:.0f}s elapsed, status: {status}")
                            if container_name:
                                container_status = self.get_container_status(container_name)
                                self.debug_log(f"Container status: {container_status}")
                ...
                consecutive_errors += 1
                self.log(f" Request timeout ({consecutive_errors}/3) - container may be building or busy...")
        ...
        self.log(f" Monitoring timeout reached after {max_wait_minutes} minutes")
        return False
    ...
    def run_simple_debug_test(self):
        ...
        try:
            ...
            try:
                response = requests.get(f"{self.server_url}/api/tasks/{self.task_id}", timeout=10)
                if response.status_code == 200:
                    result = response.json()
                    if result.get('success'):
                        task = result['task']
                        ...
                        if container_name:
                            ...
                            self.check_claude_in_container(container_name)
                            self.stream_container_logs(container_name, duration_seconds=15)
                        else:
                            self.log("No container ID found yet - container may still be starting")
                            # Wait a bit more and check again
                            time.sleep(10)
                            response = requests.get(f"{self.server_url}/api/tasks/{self.task_id}", timeout=10)
                            if response.status_code == 200:
                                updated_task = response.json().get('task', {})
                                self.log(f"Updated task status: {updated_task.get('status')}")
                                self.log(f"Updated task logs: {updated_task.get('logs', [])}")
                                
                                updated_container = updated_task.get('container_id')
                                if updated_container:
                                    self.log(f"Container now available: {updated_container}")
                                    self.check_claude_in_container(updated_container)
                                else:
                                    self.log("Still no container - checking what's happening...")
                                    self.list_claude_containers()
            ...
            # Success!
            self.log(" Simple debug test passed! Docker and Claude Code are working!")
            return True
