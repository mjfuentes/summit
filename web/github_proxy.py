...
class GitHubProxy:
    ...
    def __init__(self, github_token: str):
        ...
        self.blocked_paths = [
            '.env', '.secrets', 'credentials.json', 'private_key',
            '*.pem', '*.key', '*.p12', '*.pfx'
        ]
        
    def add_repository_access(self, repo_url: str, operations: List[str], 
                            max_commits_per_hour: int = 10, 
    ...
    def check_access(self, owner: str, repo_name: str, operation: str) -> bool:
        ...
        if len(self.operation_log[repo_key]) >= access.max_commits_per_hour:
            print(f"Rate limit exceeded for {repo_key} ({access.max_commits_per_hour}/hour)")
            return False
    ...
    def is_file_allowed(self, file_path: str, content: bytes = None) -> bool:
        ...
        if content and len(content) > self.max_file_size:
            print(f"File too large: {len(content)} bytes > {self.max_file_size}")
            return False
            
        return True
    
    def create_limited_token(self, repo_url: str, operations: List[str], 
                           expires_in_hours: int = 2) -> str:
        """Create a limited-scope token for specific operations"""
        # Add repository access
        self.add_repository_access(repo_url, operations, 
...
class SecurityConfig:
    ...
    @staticmethod
    def create_container_env(repo_url: str, task_description: str) -> Dict[str, str]:
        ...
        if any(keyword in task_description.lower() for keyword in 
               ['commit', 'push', 'save', 'update', 'fix', 'add', 'create']):
            operations.extend(['write', 'push'])
            
        if 'pull request' in task_description.lower() or 'pr' in task_description.lower():
            operations.append('create_pr')
        
        # Create limited token
        limited_token = proxy.create_limited_token(repo_url, operations, expires_in_hours=4)
    ...
    def get_claude_permissions() -> Dict:
        ...
        return {
            ...
            "allowed_commands": [
                ...
                "npm", "pip", "python", "python3", "node", "pytest",
                "black", "flake8", "mypy", "eslint", "prettier",
                "ls", "cat", "grep", "find", "echo", "which"
            ],
            "restricted_commands": [
                "git push", "git commit",  # Use proxy instead
                "sudo", "su", "rm -rf", "chmod +x",
...
if __name__ == "__main__":
    ...
    env_vars = config.create_container_env(repo_url, "Fix the authentication bug")
