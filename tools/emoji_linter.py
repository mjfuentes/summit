    ...
    def process_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if not self.auto_fix:
                print(
                    f"EMOJI VIOLATION: {file_path} contains {len(emojis_found)} emoji(s)"
                )
                return True  # Found emojis but not fixing

            # Remove emojis
            cleaned_content, emojis_removed = self.remove_emojis_from_text(
                original_content
            )
            ...
            if emojis_removed > 0:
                ...
                print(
                    f"FIXED: Removed {emojis_removed} emojis from {file_path}"
                )
