...
class CostTracker:
    ...
    def __init__(self, 
        ...
        self.output_cost_per_token = 15.0 / 1_000_000  # $15 per 1M output tokens
    ...
    def can_make_call(self, estimated_cost: float, recursion_depth: int = 0) -> tuple[bool, str]:
        """
        ...
        if recursion_depth >= self.max_recursion_depth:
            return False, f"Maximum recursion depth ({self.max_recursion_depth}) exceeded"
        
        # Check daily budget
        daily_spent = self.get_daily_spent()
        if daily_spent + estimated_cost > self.daily_budget:
            return False, f"Daily budget exceeded: ${daily_spent:.4f} + ${estimated_cost:.4f} > ${self.daily_budget}"
        
        # Check hourly budget
        hourly_spent = self.get_hourly_spent()
        if hourly_spent + estimated_cost > self.hourly_budget:
            return False, f"Hourly budget exceeded: ${hourly_spent:.4f} + ${estimated_cost:.4f} > ${self.hourly_budget}"
        
        return True, "OK"
    
    def record_call(self, input_tokens: int, output_tokens: int, 
                   model: str, recursion_depth: int = 0, 
                   call_type: str = "advice") -> float:
        """
    ...
    def cleanup_old_data(self, days_to_keep: int = 30):
        ...
        # Clean daily costs
        self.daily_costs = {k: v for k, v in self.daily_costs.items() if k >= cutoff_str}
