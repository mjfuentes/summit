...
- Infrastructure: Security, performance, deployment
- Product: User experience and business requirements  
    ...
    def _define_reviewer_personas(self) -> Dict[str, ReviewerPersona]:
        ...
        return {
            "engineer": ReviewerPersona(
                ...
                expertise=["Python", "Architecture", "Testing", "Code Quality"],
                focus_areas=[
...
Format as: ## Infrastructure Review by Jordan Kim"""
            ...
            "product": ReviewerPersona(
                ...
                expertise=["UX", "Business Logic", "Requirements", "User Impact"],
                focus_areas=[
...
Format as: ## Product Review by Sam Rodriguez"""
            ...
            "domain_expert": ReviewerPersona(
                ...
                expertise=["AI/ML", "Autonomous Systems", "Integration Patterns"],
                focus_areas=[
...
Format as: ## Domain Expert Review by Dr. Taylor Park"""
    ...
    async def get_pr_files(self, owner: str, repo: str, pr_number: int) -> Optional[List[Dict[str, Any]]]:
...
"""
    ...
    def _format_file_changes(self, files: List[Dict[str, Any]]) -> str:
        ...
        formatted = []
        for file in files[:10]:  # Limit to first 10 files for context
            status = file['status']
            ...
            # Add patch preview for small changes
    ...
    async def generate_review(self, persona_key: str, pr_info: str, file_changes: str) -> Optional[str]:
    ...
    async def post_review_comment(self, owner: str, repo: str, pr_number: int, 
                                  review_body: str, event: str = "COMMENT") -> bool:
        ...
        try:
            response = requests.post(url, headers=self.github_headers, json=payload, timeout=30)
            response.raise_for_status()
            return True
        except Exception as e:
    ...
    async def analyze_review_sentiment(self, review_text: str) -> Dict[str, Any]:
        ...
        if not self.anthropic_client or not review_text:
            return {"sentiment": "neutral", "confidence": 0.0, "reasoning": "No review text or AI client"}
...
{{
    ...
    async def conduct_multi_role_review(self, owner: str, repo: str, pr_number: int, 
                                        selected_roles: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # Format information for reviewers
        ...
        for role in selected_roles:
            if role in self.reviewers:
                ...
                review = await self.generate_review(role, pr_info, file_changes)
                ...
                if review:
                    ...
                    print(f"{role} review: {sentiment['decision']} (confidence: {sentiment['confidence']:.2f})")
        
        # Determine overall approval status
        all_approvals = all(decision == "APPROVE" for decision in review_decisions.values())
        ...
        consolidated_review = self._create_consolidated_review_with_decision(reviews, all_approvals)
        
        # Post the consolidated review
        if all_approvals and total_reviewers >= 2:  # Need at least 2 approvals
            # Post as APPROVE
            success = await self.post_review_comment(owner, repo, pr_number, consolidated_review, "APPROVE")
            decision = "APPROVED"
        elif any(decision == "REQUEST_CHANGES" for decision in review_decisions.values()):
            # Post as REQUEST_CHANGES
            success = await self.post_review_comment(owner, repo, pr_number, consolidated_review, "REQUEST_CHANGES")
            decision = "CHANGES_REQUESTED"
        else:
            # Post as COMMENT (neutral)
            success = await self.post_review_comment(owner, repo, pr_number, consolidated_review, "COMMENT")
            decision = "COMMENTED"
    ...
    def _create_consolidated_review_with_decision(self, reviews: Dict[str, Dict[str, Any]], all_approved: bool) -> str:
...
"""
        ...
        # Summary section
        approvals = sum(1 for r in reviews.values() if r["sentiment"]["decision"] == "APPROVE")
        total = len(reviews)
...
async def review_pr_with_multiple_roles(owner: str, repo: str, pr_number: int, 
                                        roles: Optional[List[str]] = None) -> Dict[str, Any]:
    """Convenience function to review a PR with multiple roles"""
    return await pr_review_system.conduct_multi_role_review(owner, repo, pr_number, roles) 