...
def monitor_agent_actions():
    ...
    print(
        f"   State: {pr_data['state']} | Mergeable: {pr_data.get('mergeable', 'unknown')}"
    )
    ...
    runs_response = requests.get(
        runs_url, headers=headers, params={"per_page": 20}
    )
    ...
    for run in runs_data["workflow_runs"]:
        ...
        is_agent_related = (
            ...
            or (
                run["event"] == "pull_request" and run_created >= pr_created
            )  # PR events since our PR
            or (
                run["head_branch"] == "main"
                and run_created >= pr_created
                and run["event"] == "push"
